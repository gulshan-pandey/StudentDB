...
public class CRUDOperations {
    ...
    public static void Delete(Statement statement, Scanner sc) throws SQLException {
        ...
        CRUDOperations.fetch(statement);                // showing all the data before deletion
...
//                sc.nextLine();                                  // to clear the buffer
    ...
    public static void BatchDelete(Connection connection, Scanner sc) throws SQLException {

        Statement batchStatement = connection.createStatement();
        CRUDOperations.fetch(batchStatement);                   // showing all the data before deletion

        System.out.println("Enter the number of rows you want to delete :");
        int rows = sc.nextInt();

        String[] BulkDelete = new String[rows];
        for(int i=0; i<rows; i++){
            System.out.println("Enter the Id of the record to delete:");
            int deleteId = sc.nextInt();

            BulkDelete[i] = "DELETE FROM stu WHERE stu_id = "+deleteId+";";
        }

        for( String query : BulkDelete){
            batchStatement.addBatch(query);
        }
        int[] ints = batchStatement.executeBatch();             // will return the number of rows changed

        for(int i =0; i<ints.length; i++){
            if(ints[i]>0) System.out.println("data deleted successfully");
            else System.out.println("something went wrong in batch processing");
        }
    }
